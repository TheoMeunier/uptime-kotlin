	...
	return (
		<div className="w-full max-w-2xl space-y-6">
			<div className="space-y-1">
				<h2 className="text-xl font-semibold">{t('profile.title.update_password')}</h2>
				<p className="text-sm text-muted-foreground">{t('profile.description.update_password')}</p>
			</div>
			...
			<form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
				<FieldGroup>
					...
					<Field>
						...
						<Input {...form.register('password_confirmation')} id="password_confirmation" type="password" required />
