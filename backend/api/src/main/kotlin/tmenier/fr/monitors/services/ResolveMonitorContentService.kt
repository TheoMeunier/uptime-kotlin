...
class ResolveMonitorContentService {
    ...
    fun resolve(request: BaseStoreProbeRequest): ProbeContent = when (request) {
        is ValidProbeProtocolHttpRequest -> ProbeContent.Http(
            url = request.url,
        ...
        is ValidProbeProtocolTcpRequest -> ProbeContent.Tcp(
            url = request.url,
            tcpPort = request.tcpPort,
        )

        is ValidProbeProtocolDnsRequest -> ProbeContent.Dns(
        ...
        is ValidProbeProtocolPingRequest -> ProbeContent.Ping(
            ip = request.ip,
