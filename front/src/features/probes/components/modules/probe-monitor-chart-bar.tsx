		...
		<div className="w-full overflow-x-auto">
			<div className="flex gap-1 my-3 h-8 items-end min-w-max">
				{Array.from({ length: emptyBars }).map((_, i) => (
					<div key={`empty-${i}`} className="h-8 w-2 flex-shrink-0 rounded bg-gray-500/40" />
				))}
							...
							<HoverCard key={item.id} openDelay={100}>
								...
								<HoverCardContent className="w-80 border-gray-300">
									<div className="space-y-2">
										...
										<div className="text-sm">The monitor was on break, no checks were performed.</div>
					...
					return (
						<HoverCard key={check.id} openDelay={100}>
							...
							<HoverCardContent className="w-80 border-gray-300">
								<div className="space-y-2">
									<div className="flex items-center justify-between">
										<h4 className="text-sm font-semibold">{getStatusLabel(check.status)}</h4>
										<span className={`text-xs px-2 py-1 rounded-full ${getStatusBadgeColor(check.status)}`}>
											{check.status}
										</span>
									</div>
									...
									{check.status === 'SUCCESS' && (
										<div className="flex items-center gap-2 text-xs text-slate-400">
											<span className="text-blue-400 font-medium">{check.response_time}ms</span>
											<span>•</span>
											<span>Response time</span>
										</div>
									)}
									<div className="pt-2 border-t border-gray-300">
										<div className="text-xs text-slate-400">{formatDate(check.run_at.toString())}</div>
										<div className="text-xs text-slate-500">{formatTime(check.run_at.toString())}</div>
									</div>
								</div>
							</HoverCardContent>
						</HoverCard>
