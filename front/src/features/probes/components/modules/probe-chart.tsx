...
export default function ProbeChart({ monitors }: { monitors: Monitor[] }) {
	...
	const getFilteredData = () => {
		...
		const intervalMinutes =
			hoursToSubtract >= 24 * 7 ? 120 : hoursToSubtract >= 24 ? 60 : hoursToSubtract >= 6 ? 30 : 15;
		...
		<Card className="pt-0">
			<CardHeader className="flex items-center gap-2 space-y-0 border-b py-5 sm:flex-row">
				<div className="grid flex-1 gap-1">
					...
					<CardDescription>Showing probe response times for the last {timeRange}</CardDescription>
				</div>
